...
{       
    ...
    {                
        ...
        $this->browse(function ($browser) use($produto){
            $browser->visit("/produtos/detalhe/{$produto->id}")          
                    ->type('nome', 'judite '.$produto->id)
                    ->type('idAtualiza', $produto->id)
                    ->press('Editar')
                    ->assertSee("Produto Atualizado de {$produto->nome} Para: {$produto->novoNome}")
                    ->assertPathIs('/produtos');
        unset($produto->novoNome);                    
        });
    }
